...
import { Plus, Search, Book, MoreHorizontal, ChevronRight, Languages, Sparkles, GraduationCap } from "lucide-react";
...
const MOCK_DICTIONARIES: DictionaryItem[] = [
  {
    ...
    count: 420,
    color: "bg-orange-500",
    icon: <Languages className="text-white size-4" />,
    lastUsed: "2 mins ago"
  },
  {
    id: "2",
    ...
    count: 85,
    color: "bg-blue-500",
    icon: <Book className="text-white size-4" />,
    lastUsed: "1 day ago"
  },
  {
    id: "3",
    ...
    count: 12,
    color: "bg-purple-500",
    icon: <GraduationCap className="text-white size-4" />,
    lastUsed: "1 week ago"
  }
];
  ...
  return (
    <div className={cn("flex flex-col h-full w-full bg-background", className)}>
      ...
      <div className="flex items-center justify-between px-1 py-4 mb-2">
        ...
        <Button size="icon" className="rounded-full h-9 w-9 shadow-sm bg-primary text-primary-foreground hover:bg-primary/90">
